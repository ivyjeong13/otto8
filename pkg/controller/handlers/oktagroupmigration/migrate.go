...
// Migrate updates group subject IDs in AccessPolicy resources
...
func (*Handler) Migrate(req router.Request, _ router.Response) error {
	...
	if err := migrateAccessPolicies(req, migration.Spec.IDMapping); err != nil {
		return err
	}
...
func migrateAccessPolicies(req router.Request, idMap map[string]string) error {
	var list v1.AccessPolicyList
	if err := req.Client.List(req.Ctx, &list); err != nil {
		return fmt.Errorf("okta group migration: failed to list AccessPolicies: %w", err)
	}
	for i := range list.Items {
		if updateSubjects(list.Items[i].Spec.Manifest.Subjects, idMap) {
			if err := req.Client.Update(req.Ctx, &list.Items[i]); err != nil {
				return fmt.Errorf("okta group migration: failed to update AccessPolicy %s: %w", list.Items[i].Name, err)
			}
		}
	}
	return nil
