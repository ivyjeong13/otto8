<script lang="ts">
	...
	let view = $state<'registry' | 'deployments'>(
		(page.url.searchParams.get('view') as 'registry' | 'deployments') || 'registry'
	);
	...
	async function switchView(newView: 'registry' | 'deployments') {
		view = newView;
...
{#snippet mainContent()}
	<div
		class="flex min-h-full flex-col gap-4 md:gap-8"
		class:pb-8={view === 'registry' && !showServerForm}
	...
	>
				...
				<div class="flex items-center">
					<button
						class={twMerge(
							...
							view === 'registry' && ' bg-blue-300 dark:bg-blue-500'
						)}
					...
					<button
						class={twMerge(
							...
							view === 'deployments' && ' bg-blue-300 dark:bg-blue-500'
						)}
		...
		{#if view === 'registry'}
			<RegistriesView bind:catalog={defaultCatalog} readonly={isAdminReadonly} {usersMap}>
				{#snippet emptyContentButton()}
					{@render addServerButton()}
				{/snippet}
			</RegistriesView>
		{:else}
			<DeploymentsView catalogId={defaultCatalogId} readonly={isAdminReadonly} {usersMap} />
		{/if}
	</div>
{/snippet}
