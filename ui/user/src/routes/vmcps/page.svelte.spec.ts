...
async function renderPageWithEntries(entries: MCPCatalogEntry[], creating = false) {
	if (creating) {
...
describe('vMCPs Page', () => {
	describe('table view', () => {
		...
		it('shows vMCPs and Sources tabs', async () => {
			await renderPageWithEntries([componentEntry, createVMcp()]);
		...
		it('takes no drops, so it offers no servers to drag', async () => {
			await renderPageWithEntries([componentEntry, createVMcp(), slack]);
			await expect
	...
	describe('create view', () => {
		...
		it('opens the designer canvas when creating a vMCP', async () => {
			await renderPageWithEntries([componentEntry, createVMcp()], true);
