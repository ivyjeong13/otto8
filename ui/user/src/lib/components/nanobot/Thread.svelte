<script lang="ts">
	...
	const lastMessageContentKey = $derived.by(() => {
		...
		const lastTextItem = last.items?.filter((i) => i.type === 'text').pop();
		const lastTextLen =
			lastTextItem && lastTextItem.type === 'text' ? (lastTextItem.text?.length ?? 0) : 0;
		return `${last.id}-${itemCount}-${lastTextLen}`;
	});

	// When content at the end changes: if we're loading, always scroll; otherwise only if user was near bottom.
	// Use double rAF so layout is complete before we read scrollHeight and scroll.
	$effect(() => {
		...
		// When loading, always auto-scroll. When not loading, only scroll if already near bottom.
