...
{#snippet vmcpCard()}
	...
	{@const name = vmcp.manifest.name ?? 'vMCP'}
	<div
	...
	>
		<VMcpCard
			id={vmcp.id}
			{name}
			descriptionHTML={vmcp.manifest.shortDescription
				? toInlineHTMLFromMarkdown(vmcp.manifest.shortDescription)
				: undefined}
			connectURL={vmcp.connectURL}
