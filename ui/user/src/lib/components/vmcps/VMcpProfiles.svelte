		...
		toolFlow.collectComponentTools(component, vmcp, (config) => {
			...
			draft.resources = draft.resources.map((resource) =>
				resource.id === id
					? {
							...resource,
							toolOverrides: (config.toolOverrides ?? []).map((tool) => ({ ...tool }))
						}
					: resource
			);
			expanded[id] = true;
...
<div class="h-full w-full overflow-y-auto p-4 pt-0">
	{#if draft}
								...
								<div class="flex items-center gap-3 py-1 pl-3 pr-1">
									...
									{#if resource}
										{#if resource.toolOverrides.length > 0}
											{@const refreshing =
												toolFlow.collecting && toolFlow.configuringComponentId === id}
