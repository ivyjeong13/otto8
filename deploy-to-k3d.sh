...
# Ensure Docker is running (k3d depends on it)
if ! docker info &>/dev/null; then
    echo "❌ Docker is not running. Start Docker Desktop, then retry."
    exit 1
fi

# Start the k3d cluster if it exists but is stopped
if ! kubectl cluster-info &>/dev/null; then
    if k3d cluster list 2>/dev/null | awk 'NR>1 {print $1}' | grep -qx "$CLUSTER_NAME"; then
        echo "⏳ k3d cluster '$CLUSTER_NAME' exists but is stopped — starting it..."
        k3d cluster start "$CLUSTER_NAME"
    else
        echo "❌ k3d cluster '$CLUSTER_NAME' not found. Create it first:"
...
if ! kubectl cluster-info &>/dev/null; then
    echo "❌ Could not connect to k3d cluster '$CLUSTER_NAME'."
    echo "   Try: k3d cluster start $CLUSTER_NAME"
    exit 1
fi
...
k3d image import obot:local -c "$CLUSTER_NAME"

echo ""
echo "📋 Deploying via Helm..."
echo "   (Helm --wait blocks until pods are Ready, up to the timeout — not frozen.)"
echo "   If this takes a while: kubectl get pods -n obot -w   # another terminal"
